...
# One of two keys
ONE_OF_TWO = "or_b(pk(tpubD6NzVbkrYhZ4XRMcMFMMFvzVt6jaDAtjZhD7JLwdPdMm9xa76DnxYYP7w9TZGJDVFkek3ArwVsuacheqqPog8TH5iBCX1wuig8PLXim4n9a/*),s:pk(tpubD6NzVbkrYhZ4WsqRzDmkL82SWcu42JzUvKWzrJHQ8EC2vEHRHkXj1De93sD3biLrKd8XGnamXURGjMbYavbszVDXpjXV2cGUERucLJkE6cy/*))"
...
class WalletMiniscriptTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.nodes[0].createwallet(
            wallet_name="ms", descriptors=True, disable_private_keys=True
        )
        funder = self.nodes[0].get_wallet_rpc(self.default_wallet_name)
        ms_wallet = self.nodes[0].get_wallet_rpc("ms")

        # Sanity check we wouldn't let an insane Miniscript descriptor in
        res = ms_wallet.importdescriptors(
            [
                {
                    "desc": descsum_create(
        ...
        assert (
            "is insecure: witnesses without signature exist" in res["error"]["message"]
        )

        for ms in MINISCRIPTS:
            self.log.info(f"Importing Miniscript '{ms}'")
            desc = descsum_create(f"wsh({ms})")
            assert ms_wallet.importdescriptors(
                [
                    {
                        "desc": desc,
                        "active": True,
            ...
            assert_equal(ms_wallet.getnewaddress(), funder.deriveaddresses(desc, 0)[0])
            assert_equal(ms_wallet.getnewaddress(), funder.deriveaddresses(desc, 1)[1])

            self.log.info("Testing we detect funds sent to one of them")
            addr = ms_wallet.getnewaddress()
            txid = funder.sendtoaddress(addr, 0.01)
            self.wait_until(
                lambda: len(ms_wallet.listunspent(minconf=0, addresses=[addr])) == 1
            )
            utxo = next(
                u
                for u in ms_wallet.listunspent(minconf=0, addresses=[addr])
                if u["txid"] == txid
            )
            assert not utxo["solvable"]  # No satisfaction logic yet
