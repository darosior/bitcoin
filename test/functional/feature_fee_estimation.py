...
class EstimateFeeTest(BitcoinTestFramework):
    ...
    def transact_and_mine(self, numblocks, mining_node):
        # We use transactions without witness data hence the minimum fee in
        # weight units is MIN_RELAY_TX_FEE / 4
        min_fee = Decimal(str(250 * 10**-8))
    ...
    def check_estimates(self):
        ...
        for i, e in enumerate(smart_estimates):  # estimate is for i+1
            ...
            if feerate + delta < min(self.fees_per_kwu) or feerate - delta > max(self.fees_per_kwu):
                raise AssertionError("Estimated fee (%f) out of range (%f,%f)"
                                     % (feerate, min(self.fees_per_kwu), max(self.fees_per_kwu)))
        ...
        smart_estimates = [self.nodes[1].estimatesmartfee(i, "ECONOMICAL", True) for i in range(1, 26)]
        for i, e in enumerate(smart_estimates):  # estimate is for i+1
            feerate = float(e["feerate"])
            assert_greater_than(feerate, 0)
    ...
    def run_test(self):
        ...
        # check_estimates(self.nodes[1], self.fees_per_kwu)
