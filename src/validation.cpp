...
{
    // New best block
    if (mempool) mempool->AddTransactionsUpdated(1);
...
/** Disconnect m_chain's tip.
  ...
  * with cs_main held.
  *
  * If disconnectpool or m_mempool is nullptr, then no disconnected transactions are added to
...
bool CChainState::ActivateBestChainStep(BlockValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexMostWork, const std::shared_ptr<const CBlock>& pblock, bool& fInvalidFound, ConnectTrace& connectTrace)
{
    ...
    while (m_chain.Tip() && m_chain.Tip() != pindexFork) {
        if (!DisconnectTip(state, chainparams, &disconnectpool)) {
            ...
            if (m_mempool) UpdateMempoolForReorg(*m_mempool, disconnectpool, false);
    ...
    if (fBlocksDisconnected) {
        ...
        if (m_mempool) UpdateMempoolForReorg(*m_mempool, disconnectpool, true);
    }
    if (m_mempool) m_mempool->check(&CoinsTip());
...
bool CChainState::InvalidateBlock(BlockValidationState& state, const CChainParams& chainparams, CBlockIndex *pindex)
{
    ...
    while (true) {
        ...
        LOCK(cs_main);
        LOCK(m_mempool ? &m_mempool->cs : nullptr); // Lock for as long as disconnectpool is in scope to make sure UpdateMempoolForReorg is called after DisconnectTip without unlocking in between
        ...
        if (m_mempool) UpdateMempoolForReorg(*m_mempool, disconnectpool, /* fAddToMempool = */ (++disconnected <= 10) && ret);
        if (!ret) return false;
...
bool CChainState::RewindBlockIndex(const CChainParams& params)
{
    ...
    while (!ShutdownRequested()) {
        {
            LOCK(cs_main);
            LOCK(m_mempool ? &m_mempool->cs : nullptr);
