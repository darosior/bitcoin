...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, BlockManager& blockman, const ChainstateManager& chainman, const CBlockIndex* pindexPrev) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    if (DeploymentActiveAfter(pindexPrev, chainman, Consensus::DEPLOYMENT_CONSENSUSCLEANUP)) max_timewarp = MAX_TIMEWARP;
    ...
    if (DeploymentActiveAfter(pindexPrev, chainman, Consensus::DEPLOYMENT_CONSENSUSCLEANUP)
        ...
        if (block.GetBlockTime() < int_start_block->GetBlockTime()) {
            return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "time-negative-interval", "the last block’s timestamp in the difficulty adjustment interval is earlier than the first block’s");
        }
    }
