...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, BlockManager& blockman, const ChainstateManager& chainman, const CBlockIndex* pindexPrev) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    if (DeploymentActiveAfter(pindexPrev, chainman, Consensus::DEPLOYMENT_CLEANUP)) max_timewarp = MAX_TIMEWARP;
    if (consensusParams.enforce_BIP94) max_timewarp = MAX_TIMEWARP_TESTNET;
    ...
    if (DeploymentActiveAfter(pindexPrev, chainman, Consensus::DEPLOYMENT_CLEANUP)
...
static bool ContextualCheckBlock(const CBlock& block, BlockValidationState& state, const ChainstateManager& chainman, const CBlockIndex* pindexPrev)
{
    ...
    // Check that all transactions are finalized and none are 64 bytes long, as those may cause the block to be
    // malleable. See https://gnusha.org/pi/bitcoindev/CAFp6fsGtEm9p-ZQF_XqfqyQGzZK7BS2SNp2z680QBsJiFDraEA@mail.gmail.com.
    const bool check_txs_size{DeploymentActiveAfter(pindexPrev, chainman, Consensus::DEPLOYMENT_CLEANUP)};
    for (const auto& tx : block.vtx) {
    ...
    // Make sure the coinbase's locktime is set to the block height minus one.
    // FIXME: should we never make it active for the genesis block instead of checking it here?
    if (nHeight > 0 && DeploymentActiveAfter(pindexPrev, chainman, Consensus::DEPLOYMENT_CLEANUP)) {
