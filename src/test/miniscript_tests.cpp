...
struct KeyConverter {
    ...
    bool IsTapscript() const {
        return m_script_ctx == miniscript::MiniscriptContext::TAPSCRIPT;
    }
};

const KeyConverter CONVERTER{/* ctx = */miniscript::MiniscriptContext::P2WSH};
const KeyConverter TAP_CONVERTER{/* ctx = */miniscript::MiniscriptContext::TAPSCRIPT};
...
void Test(const std::string& ms, const std::string& hexscript, int mode, const KeyConverter& converter, int opslimit = -1, int stacklimit = -1)
{
    ...
    if (mode == TESTMODE_INVALID || ((mode & TESTMODE_P2WSH_INVALID) && !converter.IsTapscript()) || ((mode & TESTMODE_TAPSCRIPT_INVALID) && converter.IsTapscript())) {
...
void Test(const std::string& ms, const std::string& hexscript, const std::string& hextapscript, int mode, int opslimit = -1, int stacklimit = -1)
{
    ...
    Test(ms, hextapscript == "=" ? hexscript : hextapscript, mode, TAP_CONVERTER, opslimit, stacklimit);
}
} // namespace
...
BOOST_AUTO_TEST_CASE(fixed_tests)
{
    ...
    Test("uuj:and_v(v:multi_a(2,03d01115d548e7561b15c38f004d734633687cf4419620095bc5b0f47070afe85a,025601570cb47f238d2b0286db4a990fa0f3ba28d1a319f5e7cf55c2a2444da7cc),after(1231488000))", "?", "636382926320d01115d548e7561b15c38f004d734633687cf4419620095bc5b0f47070afe85aac205601570cb47f238d2b0286db4a990fa0f3ba28d1a319f5e7cf55c2a2444da7ccba529d0400046749b168670068670068", TESTMODE_VALID | TESTMODE_NONMAL | TESTMODE_NEEDSIG | TESTMODE_P2WSH_INVALID, 14, 4);
    ...
    for (const auto subs_count: {99, 110, 200}) {
        std::string ms_str_large{"thresh(98,dv:older(99999),"};
        for (size_t i = 0; i < subs_count - 1; ++i) {
            ms_str_large += "s:pk(" + HexStr(g_testdata->pubkeys[i]) + ")";
            if (i < subs_count - 2) ms_str_large += ",";
        }
        ms_str_large += ")";
        Test(ms_str_large, "?", "?", TESTMODE_VALID | TESTMODE_NONMAL | TESTMODE_NEEDSIG | TESTMODE_P2WSH_INVALID, 5 + 3 * (subs_count - 1) + 1, subs_count);
    }
