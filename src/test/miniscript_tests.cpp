...
struct KeyConverter {
    ...
    std::vector<unsigned char> ToPKBytes(const CPubKey& key) const {
        ...
        assert(m_script_ctx == miniscript::MiniscriptContext::TAPSCRIPT);
...
{
    if (ctx == miniscript::MiniscriptContext::P2WSH) return CScript() << OP_0 << WitnessV0ScriptHash(script);
...
void SatisfactionToWitness(miniscript::MiniscriptContext ctx, CScriptWitness& witness, const CScript& script, TaprootBuilder& builder) {
    ...
    if (ctx == miniscript::MiniscriptContext::P2WSH) return;
    ...
    for (int iter = 0; iter < 3; ++iter) {
        ...
        for (int add = -1; add < (int)challist.size(); ++add) {
            ...
            if (nonmal_success) {
                ...
                const size_t max_stack_size{node->GetStackSize() + 1 + (CONVERTER.MsContext() == miniscript::MiniscriptContext::TAPSCRIPT)};
...
void Test(const std::string& ms, const std::string& hexscript, int mode, const KeyConverter& converter, int opslimit = -1, int stacklimit = -1)
{
    ...
    const bool is_tapscript{converter.MsContext() == miniscript::MiniscriptContext::TAPSCRIPT};
