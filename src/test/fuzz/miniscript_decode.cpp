...
/** A class encapulating conversion routing for CPubKey. Since we can't have access to a
...
void assertSameOps(const CScript& script_a, const CScript& script_b) {
    ...
    //std::cout << HexStr(script_a) << "  ---  " << HexStr(script_b) << std::endl;
    while (const bool res = script_a.GetOp(it_a, op_a)) {
        assert(script_b.GetOp(it_b, op_b));
        //std::cout << op_a << "  " << op_b << std::endl;
        assert(op_a == op_b);
        //std::cout << HexStr(CScript(it_a, script_a.end())) << "  ---  " << HexStr(CScript(it_b, script_b.end())) << std::endl;
    }
}
