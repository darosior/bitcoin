...
TransactionError CWallet::FillPSBT(PartiallySignedTransaction& psbtx, bool& complete, int sighash_type, bool sign, bool bip32derivs, bool finalize, size_t * n_signed) const
{
    if (n_signed) {
        *n_signed = 0;
    }
    ...
    for (ScriptPubKeyMan* spk_man : GetAllScriptPubKeyMans()) {
        ...
        TransactionError res = spk_man->FillPSBT(psbtx, txdata, sighash_type, sign, bip32derivs, finalize, &n_signed_this_spkm);
