...
// Miniscript context for satisfying ("solving") a witness script.
struct Satisfier {
    typedef CPubKey Key;
    ...
    template <typename I>
    bool FromPKBytes(I first, I last, CPubKey& pubkey) const
    {
        pubkey.Set(first, last);
        return pubkey.IsValid();
    }

    //! Conversion from a raw public key hash.
    template<typename I>
    bool FromPKHBytes(I first, I last, CPubKey& pubkey) const {
        assert(last - first == 20);
        CKeyID key_id;
        std::copy(first, last, key_id.begin());
        if (!GetPubKey(m_provider, m_sig_data, key_id, pubkey)) {
    ...
    bool CheckAfter(uint32_t value) const { return creator.Checker().CheckLockTime(CScriptNum(value)); }
    bool CheckOlder(uint32_t value) const { return creator.Checker().CheckSequence(CScriptNum(value)); }
...
public:
    ...
    bool CheckSchnorrSignature(Span<const unsigned char> sig, Span<const unsigned char> pubkey, SigVersion sigversion, const ScriptExecutionData& execdata, ScriptError* serror) const override { return sig.size() != 0; }
