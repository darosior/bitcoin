        ...
        if (node) {
            ...
            if (!node->IsSane()) {
                ...
                if (!insane_node->IsValid()) { error += " is invalid";}
                    ...
                    else if (!insane_node->CheckDuplicateKey()) error += ": contains duplicate public keys";
                    else if (!insane_node->ValidSatisfactions()) error += ": needs witnesses that may exceed resource limits";
                }
                return nullptr;
            }
