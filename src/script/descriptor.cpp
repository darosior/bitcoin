...
class ScriptMaker {
    ...
    //! The script context we're operating within (Tapscrit or P2WSH).
                                     ...
                                     FlatSigningProvider& provider) const override
    {
        for (const auto& key : keys) {
            if (script_ctx == miniscript::MiniscriptContext::TAPSCRIPT) {
                provider.pubkeys.emplace(CKeyID(Hash160(XOnlyPubKey{key})), key);
            }
...
struct KeyParser {
    ...
    template<typename I> std::optional<Key> FromPKBytes(I begin, I end) const
    {
        ...
        if (m_script_ctx == miniscript::MiniscriptContext::TAPSCRIPT && end - begin == 32) {
            ...
            m_keys.push_back(InferXOnlyPubkey(pubkey, ParseContext(), *m_in));
            return key;
...
std::unique_ptr<DescriptorImpl> ParseScript(uint32_t& key_exp_index, Span<const char>& sp, ParseScriptContext ctx, FlatSigningProvider& out, std::string& error)
{
    ...
    // Process miniscript expressions.
    {
        ...
        KeyParser parser_ctx(/*out = */&out, /* in = */nullptr, /* ctx = */script_ctx);
