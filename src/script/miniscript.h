...
template<typename Key> using NodeRef = std::shared_ptr<Node<Key>>;

//! Construct a miniscript node as a shared_ptr.
template<typename Key, typename... Args>
NodeRef<Key> MakeNodeRef(Args&&... args) { return std::make_shared<Node<Key>>(std::forward<Args>(args)...); }
...
private:
    ...
    //! Whether a public key appears more than once in this node. Only computed once actually queried.
    std::optional<bool> duplicate_key;
...
public:
    ...
    template<typename Ctx> bool ContainsDuplicateKey(const Ctx& ctx) const {
        ...
        auto upfn = [this, &ctx](const Node& node, Span<set> subs) -> std::optional<set> {
            if (&node != this && node.duplicate_key && *node.duplicate_key) return {};
...
public:
    ...
    template <typename Ctx> bool CheckDuplicateKey(const Ctx& ctx) {
        if (!duplicate_key) duplicate_key.emplace(ContainsDuplicateKey(ctx));
    ...
    template <typename Ctx> bool IsSaneSubexpression(const Ctx& ctx) {
        return ValidSatisfactions() && IsNonMalleable() && CheckTimeLocksMix() && CheckDuplicateKey(ctx);
    }

    //! Check whether this node is safe as a script on its own.
    template <typename Ctx> bool IsSane(const Ctx& ctx) {
        return IsValidTopLevel() && IsSaneSubexpression(ctx) && NeedsSignature();
    }
    ...
    Node(Fragment nt, std::vector<NodeRef<Key>> sub, std::vector<unsigned char> arg, uint32_t val = 0) : fragment(nt), k(val), data(std::move(arg)), subs(std::move(sub)), ops(CalcOps()), ss(CalcStackSize()), typ(CalcType()), scriptlen(CalcScriptLen()), duplicate_key{} {}
    Node(Fragment nt, std::vector<unsigned char> arg, uint32_t val = 0) : fragment(nt), k(val), data(std::move(arg)), ops(CalcOps()), ss(CalcStackSize()), typ(CalcType()), scriptlen(CalcScriptLen()), duplicate_key{} {}
    Node(Fragment nt, std::vector<NodeRef<Key>> sub, std::vector<Key> key, uint32_t val = 0) : fragment(nt), k(val), keys(std::move(key)), subs(std::move(sub)), ops(CalcOps()), ss(CalcStackSize()), typ(CalcType()), scriptlen(CalcScriptLen()), duplicate_key{} {}
    Node(Fragment nt, std::vector<Key> key, uint32_t val = 0) : fragment(nt), k(val), keys(std::move(key)), ops(CalcOps()), ss(CalcStackSize()), typ(CalcType()), scriptlen(CalcScriptLen()), duplicate_key{} {}
    Node(Fragment nt, std::vector<NodeRef<Key>> sub, uint32_t val = 0) : fragment(nt), k(val), subs(std::move(sub)), ops(CalcOps()), ss(CalcStackSize()), typ(CalcType()), scriptlen(CalcScriptLen()), duplicate_key{} {}
    Node(Fragment nt, uint32_t val = 0) : fragment(nt), k(val), ops(CalcOps()), ss(CalcStackSize()), typ(CalcType()), scriptlen(CalcScriptLen()), duplicate_key{} {}
};
