...
inline NodeRef<Key> Parse(Span<const char> in, const Ctx& ctx)
{
    ...
    // Parse a multi() or multi_a() from its string representation. Returns false on parsing error.
    ...
    while (!to_parse.empty()) {
        ...
        case ParseContext::EXPR: {
            if (Const("0", in)) {
                ...
                if (!parse_multi_exp(in, false)) return {};
            } else if (Const("multi_a(", in)) {
                if (!parse_multi_exp(in, true)) return {};
...
inline NodeRef<Key> DecodeScript(I& in, I last, const Ctx& ctx)
{
    ...
    while (!to_parse.empty()) {
        ...
        case DecodeContext::SINGLE_BKV_EXPR: {
            ...
            // Tapscript's multi
            if (last - in >= 3 && in[0].first == OP_NUMEQUAL) {
