...
struct Ops {
    ...
    static uint32_t inline K(uint32_t x) { return x; }

    static uint32_t inline Add(uint32_t x, uint32_t y) { return x + y; }
    static uint32_t inline Xor(uint32_t x, uint32_t y) { return x ^ y; }
    static uint32_t inline Or(uint32_t x, uint32_t y) { return x | y; }
    static uint32_t inline And(uint32_t x, uint32_t y) { return x & y; }
    static uint32_t inline ShR(uint32_t x, int n) { return x >> n; }
    static uint32_t inline ShL(uint32_t x, int n) { return x << n; }

    static uint32_t inline Read(const unsigned char* chunk, int offset) {
        return ReadBE32(chunk + offset);
    }

    static void inline Write(unsigned char* out, int offset, uint32_t n) {
        WriteBE32(out + offset, n);
    }
};
